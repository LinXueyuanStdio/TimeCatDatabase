...
import com.timecat.data.system.model.DateFormats;
import com.timecat.data.system.model.DateUtils;
...
public abstract class CheckmarkList {
    ...
    static List<Checkmark> buildCheckmarksFromIntervals(Repetition[] reps,
        ...
        for (int i = 0; i < nDays; i++)
            checkmarks.add(new Checkmark(today.minus(i), Checkmark.UNCHECKED));
        ...
        for (Interval interval : intervals) {
            for (int i = 0; i < interval.length(); i++) {
                ...
                checkmarks.set(offset, new Checkmark(date, Checkmark.CHECKED_IMPLICITLY));
            }
        }
        ...
        for (Repetition rep : reps) {
            ...
            int offset = date.daysUntil(today);
            checkmarks.set(offset, new Checkmark(date, Checkmark.CHECKED_EXPLICITLY));
        }
     ...
     */
    @NonNull
    private static ArrayList<Interval> buildIntervals(@NonNull Frequency freq,
